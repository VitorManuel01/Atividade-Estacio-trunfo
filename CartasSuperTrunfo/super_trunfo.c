...
float calcularDensidade(CartaCidade *cartacidade)
{
    ...
    return cartacidade->populacao / cartacidade->area;
}

float calcularPIBPercapita(CartaCidade *cartacidade){
    return (cartacidade->pib * 1000) / cartacidade->populacao;
}
...
void mostrarCartas(CartaCidade *cartacidade, int qtdCartas)
{
    ...
    for (int i = 0; i < qtdCartas; i++)
    {
        ...
        printf("Densidade Populacional: %.2f hab/km²\n", calcularDensidade(cartacidade));

        printf("PIB per Capita: R$%.2f\n\n\n", calcularPIBPercapita(cartacidade));
    }
}
